...
class RMEvalManager:
    ...
    def __call__(self, data: DataProto):
        ...
        for i in range(len(data)):
            ...
            # [수정] compute_score 관련 로직 전면 제거 및 새 공식 적용
            
            # 1. Model Eval Score 가져오기 (API 결과)
            model_eval_score = next(eval_results_iter)
            
            # 2. RAG Metric 계산 (Recall, NDCG)
            ...
            recall_val = recall_ret(retrievaled_images_basename_list, reference_images_basename_list)
            ndcg_value = ndcg(retrievaled_images_basename_list, reference_images_basename_list)
            
            recall_list.append(recall_val)
            ndcg_list.append(ndcg_value)

            # 3. 최종 점수 계산 (0.2 * model + 0.8 * ndcg)
            ...
            if already_print_data_sources[data_source] < self.num_examine:
                ...
                print(f"[model_score] {model_eval_score}, [ndcg] {ndcg_value}") # 로그 수정
